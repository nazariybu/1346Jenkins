        ...
        stage('Check Apache Logs') {
            steps {
                script {
                    ...
                    def errors = sh(script: 'grep -E " 4[0-9]{2} | 5[0-9]{2} " /var/log/apache2/access.log | wc -l', returnStdout: true).trim()
                    if (errors != "0") {
                        echo "Знайдено помилки в логах Apache!"
                        // Можна вивести деталі:
                        sh 'grep -E " 4[0-9]{2} | 5[0-9]{2} " /var/log/apache2/access.log'
